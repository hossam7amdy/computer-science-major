...
public:
	...
	/// 3) Print all duplicate subtrees TODO
    string subtree(const BinaryTree* sub){
        string ret = "(" + toStr(data);

        if(sub->left)
            ret += subtree(sub->left);
        else
            ret += "()";
        if(sub->right)
            ret += subtree(sub->right);
        else
            ret += "()";
        ret += ")";
	...
	void print_all_duplicates(){
        ...
        if(left)
            v.push_back(left->subtree(left));
        if(right)
            v.push_back(right->subtree(right));
